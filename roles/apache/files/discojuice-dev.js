(function(a) {
    function c(c) {
        ...
        var l = this, m, k = {}, n = c ? a.fn : a, p = arguments, t = 4, q = p[1], s = p[2], r = p[3];
        "string" !== typeof q && (t--,
        q = c = 0,
        s = p[1],
        r = p[2]);
        c ? (m = l.eq(0),
        m.data(c, k = m.data(c) || {})) : q && (k = d[q] || (d[q] = {}));
        k.id && clearTimeout(k.id);
        delete k.id;
        if (r)
            k.fn = function(a) {
                "string" === typeof r && (r = n[r]);
                !0 !== r.apply(l, e.call(p, t)) || a ? h() : g()
            }
            ,
            g();
        else {
            if (k.fn)
                return void 0 === s ? h() : k.fn(!1 === s),
                !0;
            h()
        }
    }
...
DiscoJuice.Utils = {
    murmurhash3_32_gc: function(a, c) {
        ...
        f = c;
        for (g = 0; g < e; )
            h = a.charCodeAt(g) & 255 | (a.charCodeAt(++g) & 255) << 8 | (a.charCodeAt(++g) & 255) << 16 | (a.charCodeAt(++g) & 255) << 24,
            ++g,
        ...
        case 3:
            h ^= 
            (a.charCodeAt(g + 2) & 255) << 16;
        case 2:
            h ^= (a.charCodeAt(g + 1) & 255) << 8;
        case 1:
            h ^= a.charCodeAt(g) & 255,
            h = 3432918353 * (h & 65535) + ((3432918353 * (h >>> 16) & 65535) << 16) & 4294967295,
            h = h << 15 | h >>> 17,
    ...
    options: function() {
        ...
        return {
            get: function(c, 
            d) {
                return a && "undefined" !== typeof a[c] ? a[c] : d
            },
            set: function(c) {
                a = c
            },
    ...
    readCookie: function(a) {
        a = "_DiscoJuice_" + (a || "EntityID") + "=";
        for (var c = document.cookie.split(";"), d = 0; d < c.length; d++) {
            for (var e = c[d]; " " == e.charAt(0); )
                e = e.substring(1, e.length);
            if (0 == e.indexOf(a))
                return unescape(e.substring(a.length, e.length))
        }
        return null 
    },
    ...
    searchMatch: function(a, c) {
        ...
        if (a.keywords)
            for (d in a.keywords)
                for (f = a.keywords[d],
                e = 0; e < f.length; e++)
                    if (-1 !== f[e].toLowerCase().search(c.toLowerCase()))
                        return f[e];
        return !1
    },
    calculateDistance: function(a, c, d, e) {
        var f = this.toRad(d - a);
        c = this.toRad(e - c);
        a = Math.sin(f / 2) * Math.sin(f / 2) + Math.cos(this.toRad(a)) * Math.cos(this.toRad(d)) * Math.sin(c / 2) * Math.sin(c / 2);
    ...
    waiter: function(a, c) {
        function d() {
            h ? console.log("Execution cancelled. Already performed.") : 
            (h = !0,
            a(e))
        }
        var e = {}
        ...
        e.startTimer = function() {
            0 === f.length ? (console.log("Executing because no action is scheduled...."),
        ...
        e.runAction = function(a, c) {
            ...
            a(function() {
                ...
                h ? e.allowMultiple ? (console.log("Slow response; but executing anyway!!"),
                d()) : "function" === typeof c && (console.log("All actions completed. Too late for executing..."),
                c()) : (console.log("All actions completed. Executing!"),
                d())
            })
        }
        ;
...
DiscoJuice.UI = {
    ...
    sprintf: function() {
        if (arguments && !(1 > arguments.length) && RegExp) {
            for (var a = 
            arguments[0], c = /([^%]*)%('.|0|\x20)?(-)?(\d+)?(\.\d+)?(%|b|c|d|u|f|o|s|x|X)(.*)/, d = b = [], e = 0, f = 0; d = c.exec(a); ) {
                var a = d[1]
                  , h = d[2]
                  , g = d[4]
                  , l = d[5]
                ...
                if ("%" == m)
                    ...
                    var k = arguments[e]
                      , n = "";
                    h && "'" == h.substr(0, 1) ? n = a.substr(1, 1) : h && (n = h);
                    h = -1;
                    g && (h = parseInt(g));
                    g = -1;
                    l && "f" == m && (g = parseInt(l.substring(1)));
                    l = k;
                    switch (m) {
                    case "b":
                        l = parseInt(k).toString(2);
                        break;
                    case "c":
                        l = String.fromCharCode(parseInt(k));
                        break;
                    case "d":
                        l = parseInt(k) ? parseInt(k) : 0;
                        break;
                    case "u":
                        l = Math.abs(k);
                        break;
                    case "f":
                        l = -1 < g ? Math.round(parseFloat(k) * Math.pow(10, g)) / Math.pow(10, g) : parseFloat(k);
                        break;
                    case "o":
                        l = parseInt(k).toString(8);
                        break;
                    case "s":
                        l = k;
                        break;
                    case "x":
                        l = ("" + parseInt(k).toString(16)).toLowerCase();
                        break;
                    case "X":
                        l = ("" + parseInt(k).toString(16)).toUpperCase()
                    }
                    m = h - l.toString().length;
                    n = 0 < m ? Array(m + 1).join(n ? n : " ") : ""
                }
                a = 
                a + n + l + d
            }
    ...
    addItem: function(a, c, d, e, f, h) {
        var g = "";
        h = h ? "enabled" : "disabled";
        var l = this.parent.Utils.options.get("discoPath", "") + "logos/"
          , m = this.parent.Utils.options.get("discoPath", "") + "flags/"
          , k = !1
          , n = this.parent.Utils.options.get("debug.weight", !1)
          , p = a.entityID;
        a.subID && (p += "#" + a.subID);
        this.alreadyLoaded[p] || (this.alreadyLoaded[p] = !0,
        a.icon && this.parent.Utils.options.get("showIcon", !0) && (g += '<img class="logo" src="' + l + a.icon + '" />',
        k = !0),
        f && (g += '<span style="font-size: 80%; -webkit-border-radius: 3px; -moz-border-radius: 3px; border-radius: 3px; border: 1px solid #ccc; background: #eee; color: #777; padding: 3px 2px 0px 2px; margin: 3px; float: left; left: -10px">&#8629;</span>'),
        g += '<span class="title">' + a.title + "</span>",
        d && !0 !== d ? g += '<span class="substring">&#8212; ' + d + "</span>" : a.descr && (g += '<span class="substring">&#8212; ' + a.descr + "</span>"),
        (c || void 0 != e) && this.parent.Utils.options.get("showLocationInfo", !0) && (g += '<span class="location">',
        c && (g += '<span class="country">',
        c.flag && (g += '<img src="' + m + c.flag + '" alt="' + escape(c.country) + '" /> '),
        g += c.country + "</span>"),
        void 0 != e && (g = 1 > e ? g + ('<span class="distance">' + DiscoJuice.Dict.nearby + "</span>") : g + ('<span class="distance">' + 
        Math.round(e) + " km</span>")),
        g += "</span>"),
        n && (g += '<div class="debug">',
        a.subID && (g += '<input value="' + a.subID + '" />'),
        c = 0,
        a.weight && (c += a.weight),
        a.distanceweight && (c += a.distanceweight),
        g += 'Weight <strong style="color: #888">' + Math.round(100 * c) / 100 + "</strong> ",
        a.weight && (g += " (base " + a.weight + ")   "),
        a.distanceweight && (g += "(dist " + Math.round(100 * a.distanceweight) / 100 + ")"),
        g += "</div>"),
        k && (g += '<hr style="clear: both; height: 0px; visibility:hidden" />'),
        g = '<a href="" class="' + h + '" rel="' + escape(p) + 
        '" title="' + DiscoJuice.Utils.escapeHTML(a.title) + '">' + g + "</a>",
        this.resulthtml += g)
    },
    ...
    refreshData: function(a, c, d) {
        ...
        a ? (a = '<a class="discojuice_showmore textlink" href="">' + this.sprintf(DiscoJuice.Dict.moreLink, c, this.parent.Control.data.length) + "\u2026</a>",
        this.popup.find("p.discojuice_moreLinkContainer").empty().append(a),
        this.popup.find("p.discojuice_moreLinkContainer a.discojuice_showmore").click(function(a) {
            a.preventDefault();
            e.control.increase()
        })) : (this.popup.find("p.discojuice_moreLinkContainer").empty(),
        10 < d && (a = '<span style="color: #888">' + d + " entries listed</span>",
        this.popup.find("p.discojuice_moreLinkContainer").append(a)))
    },
    error: function(a) {
        console.log("error" + a);
    ...
    enable: function(a) {
        var c = this.parent.Utils.options.get("discoPath", "") + "images/"
          ...
          , e = this.parent.Utils.options.get("textHelp", 
        DiscoJuice.Dict.help)
          , f = this.parent.Utils.options.get("textHelpMore", DiscoJuice.Dict.helpMore)
          , h = this.parent.Utils.options.get("subtitle", null )
          , h = null  !== h ? '<p class="discojuice_subtitle">' + h + "</p>" : ""
          , c = '<div style="display: none" class="discojuice"><div class="top"><a href="#" class="discojuice_close">&nbsp;</a><p class="discojuice_maintitle">' + this.parent.Utils.options.get("title", "Title") + "</p>" + h + '</div><div class="discojuice_listContent" style=""><div class="scroller"><div class="loadingData" ><img src="' + 
        ...
        g = this
          , d = $("body");
        this.parent.Utils.options.get("useTarget", !1) && (d = this.parent.Utils.options.get("target", d));
        console.log("Target is");
        console.log(d);
        this.parent.Utils.options.get("overlay", !0) && (console.log("DiscoJuice Enable: adding overlay"),
        $('<div id="discojuice_overlay" style="display: none"></div>').appendTo($("body")));
        this.popup = $(c).appendTo(d);
        !0 === this.parent.Utils.options.get("always", !1) ? (this.popup.find(".discojuice_close").hide(),
        this.show()) : $(a).click(function(a) {
            a.preventDefault();
            g.show();
            return !1
        });
        this.popup.find("p#dj_help").click(function() {
            g.setScreen("<h2>" + DiscoJuice.Dict.about + '</h2><p style="margin: .5em 0px">' + g.sprintf(DiscoJuice.Dict.aboutDescr, '<a href="http://uninett.no">', "</a>") + '</p><p style="margin: .5em 10px"><a href="http://discojuice.org" target="_blank">' + DiscoJuice.Dict.aboutMore + '</a></p><p style="margin: .5em 0px; font-size: 80%">' + DiscoJuice.Dict.version + ": " + DiscoJuice.Version)
        });
        this.popup.find("#discojuiceextesion_listener").click(function() {
            g.control.discojuiceextension()
        });
        this.popup.find("#discojuice_page_return input").click(function(a) {
            a.preventDefault();
            g.returnToProviderList()
        });
        this.popup.find(".discojuice_close").click(function() {
            g.hide()
        });
        this.popup.find(".discojuice_what").click(function() {
            g.popup.find(".discojuice_whatisthis").toggleClass("show")
        });
        this.parent.Utils.options.get("location", !1) && navigator.geolocation && (g = this,
        $("a#locateme").click(function(a) {
            var c = g.parent.Utils.options.get("discoPath", "") + "images/";
            g.parent.Utils.log("Locate me. Detected click event.");
            a.preventDefault();
            a.stopPropagation();
...
DiscoJuice.Control = {
    ...
    runCallback: function(a) {
        if (this.wncr[a] && "function" === typeof this.wncr[a])
            this.wncr[a]()
    },
    load: function() {
        var a = this;
        if (!this.data) {
            ...
            d = this.parent.Utils.options.get("metadata"), e = [], f = {}, h = null , g;
            ...
            if (d) {
                ...
                this.parent.Utils.options.get("discoSetRequestor", !1) && (f.entityID = c.spentityid);
                ...
                for (g = 0; g < e.length; g++)
                    h = 
                    e[g],
                    c.runAction(function(c) {
                        var d = g + 1;
                        $.ajax({
                            url: h,
                            dataType: "jsonp",
                            jsonpCallback: function() {
                                return "_" + a.parent.Utils.murmurhash3_32_gc(h, 0).toString(36)
                            },
                            cache: !0,
                            data: f,
                            success: function(f) {
                        ...
                        var c = h;
                        return function() {
                            a.ui.error("Metadata retrieval from [" + c + "] to slow. Ignoring response.")
                        }
                    }());
                c.startTimer()
            }
    ...
    postLoad: function() {
        ...
        if (this.data) {
            for (i = 
            0; i < this.data.length; i++)
                !this.data[i].title && this.data[i].DisplayNames && (this.data[i].title = this.data[i].DisplayNames[0].value);
    ...
    readExtensionResponse: function() {
        this.extensionResponse && (this.extensionResponse.autologin && this.selectProvider(this.extensionResponse.entityID, this.extensionResponse.subID),
        this.extensionResponse.selectedRelID && this.setWeight(-100, this.extensionResponse.entityID, 
        this.extensionResponse.subID),
    ...
    setWeight: function(a, c, d) {
        for (i = 0; i < this.data.length; i++)
            if (!(this.data[i].entityID !== c || d && !this.data[i].subID || d && d !== this.data[i].subID || this.data[i].subID && !d)) {
    ...
    discoResponseError: function(a, c) {
        this.parent.Utils.log("DiscoResponse ERROR Received cid=" + 
        a);
        a && this.runCallback(a);
        c && this.ui.error(c)
    },
    discoResponse: function(a, c, d, e) {
        this.parent.Utils.log("DiscoResponse Received from [" + a + "] entityID: " + c + " subID: " + d);
        a = !0;
        var f = this.parent.Utils.options.get("disco");
        f && (f = f.subIDstores) && f[c] && !d && (this.parent.Utils.log("Ignoring discoResponse from entityID: " + c + " because subID was required and not provided"),
        a = !1);
        a && this.setWeight(-100, c, d);
        ...
        for (var c, d, e = 0; e < this.data.length; e++)
            if (this.data[e].geo) {
                ...
                for (var f = 0; f < c.length; f++)
                    d.push(this.parent.Utils.calculateDistance(c[f].lat, c[f].lon, this.location[0], this.location[1]));
    ...
    locateMe: function() {
        ...
        navigator.geolocation ? navigator.geolocation.getCurrentPosition(function(c) {
            a.ui.setLocationText("You are here: " + 
            c.coords.latitude + ", " + c.coords.longitude + ". Nearby providers shown on top.");
    ...
    prepareData: function(a) {
        ...
        var c, d, e, f = !1, h = this.getTerm(), g = this.getCategories();
        ...
        if (this.data) {
            ...
            h || g ? this.ui.popup.find("p.discojuice_showall").show() : this.ui.popup.find("p.discojuice_showall").hide();
            this.ui.clearItems();
            var l = !1;
            for (c = a = 0; c < this.data.length; c++)
                ...
                d.title) {
                    if (h) {
                        if (e = this.parent.Utils.searchMatch(d, h),
                        !1 === e)
                            continue
                    } else
                        e = null ;
                    if (g && g.country) {
                        if (!d.country)
                            continue;if ("_all_" !== d.country && g.country !== d.country && -50 < d.weight)
                            continue
                    }
                    if (++a > this.maxhits) {
                        f = !0;
                    ...
                    if (d.country) {
                        var k = 
                        this.parent.Constants.Countries[d.country] ? this.parent.Constants.Countries[d.country] : d.country;
                        "_all_" !== k && (m = {
                            country: k,
                    ...
                    k = !1;
                    l || (h && !1 !== e ? l = k = !0 : h || (l = k = !0));
                    var n = this.isEnabled(d);
                    this.ui.addItem(d, m, e, d.distance, k, n);
                    k && (this.quickEntry = d)
                } else
                    this.parent.Utils.log("No title for this entry [" + d.entityID + d.relID + "] skipping.");
            this.ui.refreshData(f, this.maxhits, a)
        }
    },
    hitEnter: function() {
    ...
    selectProvider: function(a, c) {
        ...
        for (i = 0; i < this.data.length; i++)
            this.data[i].entityID == a && (c && c != this.data[i].subID || (f = this.data[i]));
        if (f.auth && "local" === f.auth)
            ...
            if (this.parent.Utils.options.get("cookie", !1)) {
                var g = a;
                c && (g += "#" + c);
                this.parent.Utils.log("COOKIE write " + g);
                this.parent.Utils.createCookie(g)
            }
            this.parent.Utils.log("Entity Selected");
            this.parent.Utils.log(f);
    ...
    discoReadSetup: function(a) {
        ...
        if (d) {
            var e = "", f = d.url, h = d.spentityid, g = d.stores, l, m, k, n;
            if (g)
                for (l = 0; l < g.length; l++)
                    a.runAction(function(a) {
                        k = c.registerCallback(a);
                        n = DiscoJuice.Utils.addQueryParam(f, "cid", k);
                        m = g[l];
                        c.parent.Utils.log("Setting up DisoJuice Read from Store [" + m + "]");
                        iframeurl = m + "?entityID=" + escape(h) + "&isPassive=true&returnIDParam=entityID&return=" + escape(n);
    ...
    discoSubReadSetup: function(a) {
        ...
        if (c) {
            var e = "", f = c.url, h = c.spentityid, g = c.subIDstores, l, m;
            if (g)
                for (var k in g)
                    this.parent.Utils.log("discoSubReadSetup()"),
                    a.runAction(function(a) {
                        m = d.registerCallback(a);
                        f = c.url + "?entityID=" + escape(k) + "&cid=" + m;
                        l = g[k];
                        d.parent.Utils.log("Setting up SubID DisoJuice Read from Store [" + k + 
                        "] =>  [" + l + "]");
                        iframeurl = l + "?entityID=" + escape(h) + "&isPassive=true&returnIDParam=subID&return=" + escape(f);
                        d.parent.Utils.log("iFrame URL is  [" + iframeurl + "]");
                        d.parent.Utils.log("return URL is  [" + f + "]");
                        e = '<iframe src="' + iframeurl + '" style="display: none"></iframe>';
                        d.ui.addContent(e)
                    })
        }
    ...
    discoWrite: function(a, c) {
        ...
        if (c) {
            this.parent.Utils.log("DiscoJuice.Control discoWrite(...)");
            if (d.subIDwritableStores && d.subIDwritableStores[a])
                return this.parent.Utils.log("DiscoJuice.Control discoWrite(" + a + ") with SubID [" + c + "]"),
                iframeurl = d.subIDwritableStores[a] + escape(c),
                this.parent.Utils.log("DiscoJuice.Control discoWrite iframeURL (" + iframeurl + ") "),
                e = '<iframe src="' + iframeurl + '" style="display: none"></iframe>',
                this.ui.addContent(e),
                !0;
            g += "#" + c
        }
        this.parent.Utils.log("DiscoJuice.Control discoWrite(" + g + ") to " + h);
        iframeurl = 
        DiscoJuice.Utils.addQueryParam(h, "origin", f);
        iframeurl = DiscoJuice.Utils.addQueryParam(iframeurl, "IdPentityID", g);
    ...
    searchboxSetup: function() {
        ...
        this.ui.popup.find("input.discojuice_search").keydown(function(d) {
            var e;
            d && d.which ? e = d.which : window.event && (d = window.event,
            e = d.keyCode);
    ...
    filterCountrySetup: function(a) {
        ...
        var e = {};
        for (d in this.data)
            this.data[d].country && "_all_" !== this.data[d].country && (e[this.data[d].country] = !0);
        var f = 0;
        for (d in e)
            f++;
        f = this.parent.Utils.options.get("setCountry");
        !a && f && filterOptions[f] && (a = f);
        f = '<p class="discojuice_filter_country">' + DiscoJuice.Dict.showIn + ' <select class="discojuice_filterCountrySelect" name="filterCountrySelect">';
        f = a ? f + ('<option value="all">' + DiscoJuice.Dict.allCountries + "</option>") : f + ('<option value="all" selected="selected">' + DiscoJuice.Dict.allCountries + "</option>");
        for (d in this.parent.Constants.Countries)
            d === a ? f += '<option value="' + d + '" selected="selected">' + this.parent.Constants.Countries[d] + "</option>" : e[d] && (f += '<option value="' + d + '" >' + this.parent.Constants.Countries[d] + "</option>");
        f = f + "</select>" + (' <a class="discojuice_showall textlink" href="">' + DiscoJuice.Dict.showAllCountries + "</a>");
        f += "</p>";
        this.ui.addFilter(f).find("select").change(function(a) {
            a.preventDefault();
            c.resetTerm();
            c.ui.focusSearch();
    ...
    getCountry: function(a) {
        ...
        if (c) {
            var e = this.parent.Utils.readCookie("Country2")
              , f = parseFloat(this.parent.Utils.readCookie("GeoLat"))
              , h = parseFloat(this.parent.Utils.readCookie("GeoLon"));
            e ? (this.setCountry(e, !1),
            this.parent.Utils.log("DiscoJuice getCountry() : Found country in cache: " + 
            e),
            f && h && this.setPosition(f, h, !1)) : a.runAction(function(a) {
                $.ajax({
                    cache: !0,
                    url: c,
                    dataType: "jsonp",
                    ...
                    success: function(c) {
                        c && "ok" == c.status && c.country ? (d.parent.Utils.createCookie(c.country, "Country2"),
                        d.setCountry(c.country, !1),
                        d.parent.Utils.log("DiscoJuice getCountry() : Country lookup succeeded: " + c.country),
                        c.geo && c.geo.lat && c.geo.lon && (d.setPosition(c.geo.lat, c.geo.lon, !1),
                        d.parent.Utils.createCookie(c.geo.lat, "GeoLat"),
                        d.parent.Utils.createCookie(c.geo.lon, 
                        "GeoLon"))) : c && c.error ? (d.parent.Utils.log("DiscoJuice getCountry() : Country lookup failed: " + (c.error || "")),
                        d.ui.error("Error looking up users localization by country: " + (c.error || ""))) : (d.parent.Utils.log("DiscoJuice getCountry() : Country lookup failed"),
                        d.ui.error("Error looking up users localization by country."));
                        a()
                    }
                })
            })
    ...
    getCategories: function() {
        var a = {}, c;
        (c = this.ui.popup.find("select.discojuice_filterTypeSelect").val()) && 
        "all" !== c && (a.type = c);
        (c = this.ui.popup.find("select.discojuice_filterCountrySelect").val()) && "all" !== c && (a.country = c);
        return a
    },
    getTerm: function() {
...
function getConfig(a) {
    ...
    a.hasOwnProperty("title") && (c.title = "Sign in to <strong>" + a.title + "</strong>",
    c.subtitle = "Select your Provider");
    a.hasOwnProperty("spentityid") && a.hasOwnProperty("responseurl") && 
    (c.disco = {
    ...
    a.hasOwnProperty("redirectURL") && (c.callback = function(c, d) {
        window.location = a.redirectURL + escape(c.entityID)
    }
    );
    if (a.hasOwnProperty("feeds"))
        for (d = 0; d < a.feeds.length; d++)
            c.metadata.push("https://cdn.discojuice.org/feeds/" + a.feeds[d]);
    return c
}
DiscoJuice.Hosted = {
    getConfig: getConfig,
    setup: function(a) {
        var c, d = "body";
        c = getConfig(a);
        a.hasOwnProperty("target") ? d = a.target : c.always = !0;
        $(document).ready(function() {
            $(d).DiscoJuice(c)
        })
    }
};
DiscoJuice.Version = "3.0 (2013-09-25 21:35)";
"undefined" == typeof DiscoJuice && (DiscoJuice = {});
DiscoJuice.Dict = {
